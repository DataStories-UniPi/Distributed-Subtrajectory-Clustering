...
public class DSCDriver {
    ...
    public static void main(String[] args) throws Exception {
     	...
     	threshSim = Double.parseDouble(args[16]);
     	minVoting = Double.parseDouble(args[17]);

     	final String job_description = args[18];
		...
		job1.setJarByClass(DSCDriver.class);
		...
		job1.setReducerClass(DTJ2ClustReducer.class);
		...
		job2.setNumReduceTasks(nof_reducers);
		
		job2.setJarByClass(DTJ2ClustDriver.class);
		job2.setPartitionerClass(DTJ2ClustPartitioner.class);
		job2.setGroupingComparatorClass(DTJ2ClustGroupingComparator.class);
		
		job2.setMapperClass(DTJ2ClustMapper.class);

		job2.setReducerClass(DTJ2ClustRefineReducer.class);


		job2.setInputFormatClass(MyMergePartitionFileInputFormat.class);

		job2.setMapOutputKeyClass(DTJSubtrajIntersectPart.class);
		job2.setMapOutputValueClass(DTJSubtrajPairSim.class);
		job2.setOutputKeyClass(DTJSubtraj.class);
		job2.setOutputValueClass(DTJSubtrajSim.class);
	
		MyMergePartitionFileInputFormat.setInputPaths(job2, new Path(hostname.concat(dfs_port).concat(workspace_dir).concat(intermdt_output_dir)));
